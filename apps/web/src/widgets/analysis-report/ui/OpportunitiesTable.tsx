...
const columns = columnHelper.columns([
  ...
  columnHelper.accessor("score", {
    header: ({ column }) => (
      <SortHeader
        label="Оценка"
        help="Сумма оценок пяти лучших фрагментов страницы. Оценка фрагмента — релевантность × (1 − сходство), обе величины косинусные: релевантность до эмбеддинга запроса, сходство до ближайшего фрагмента вашего домена. Берутся пять лучших, а не все, иначе длинная статья не по теме обгоняет короткую по теме за счёт объёма. Максимум — 5."
        direction={column.getIsSorted()}
        onSort={() => column.toggleSorting()}
      />
    ),
