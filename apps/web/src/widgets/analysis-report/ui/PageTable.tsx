  ...
  columnHelper.accessor("priority", {
    header: ({ column }) => (
      <SortHeader
        ...
        note="Приоритетность страницы как возможного нового покрытия для вашего домена: высока, только когда страница одновременно о запросе и не похожа на то, что у вас уже есть. Относительность наследует от новизны. Процент здесь — признак относительности, долей чего-либо приоритет не является."
      />
    ),
    sortUndefined: "last",
  ...
  columnHelper.accessor("pageDate", {
    header: ({ column }) => (
      <SortHeader
        label="Дата"
        column={column}
        formula="дата из разметки страницы, при её отсутствии — lastmod карты сайта"
        note="Из разметки берутся article:published_time, datePublished в JSON-LD и time[datetime] — точный источник виден при наведении на дату. Пометка lastmod означает, что разметка даты не дала и взято значение из карты сайта: оно часто означает пересборку страницы, а не публикацию. На расчёт дата не влияет."
      />
    ),
    sortUndefined: "last",
